...
\begin{abstract}
    Low-discrepancy (LD) sequences, also called quasi-random sequences, have been extensively used as efficient experimental designs across many scientific disciplines. This article provides a unified description and software interface for methods pertaining to LD point sets. These methods include LD point set generators, randomization techniques, and fast kernel methods. Specifically, we provide generators for lattices, digital nets, and Halton point sets. Supported randomization techniques include random permutations / shifts, linear matrix scrambling, and nested uniform scrambling. Routines for working with higher-order digital nets and scramblings are also detailed. For kernel methods, we provide implementations of shift-invariant (SI) and digitally-shift-invariant (DSI) kernels, which, when paired with lattices and digital nets respectively, yield structured Gram for which operations may be performed quickly. These fast Gram matrix operations are facilitated by the fast Fourier transform in bit-reversed order (FFTBR), the inverse fast Fourier transform in bit-reversed order (IFFTBR), and the fast Walsh--Hadamard transform (FWHT) algorithms which are also implemented. We present methods to quickly update Gram matrix multiplication and inversion after doubling the number of points in a lattice or digital net in radical inverse order. A new DSI kernel of higher-order smoothness is also derived. 
\end{abstract}
...
Low-discrepancy (LD) sequences, also called quasi-random sequences, judiciously explore the unit cube in high dimensions. They were first developed to replace the independent points used in Monte Carlo simulation for high dimensional numerical integration. The resulting Quasi-Monte Carlo (QMC) methods have been proven to be theoretically superior to independent Monte Carlo methods for a large class of nicely behaved functions \citep{niederreiter.qmc_book,dick.digital_nets_sequences_book,kroese.handbook_mc_methods,dick2022lattice,lemieux2009monte,sloan1994lattice,dick.high_dim_integration_qmc_way}. This advantage has been shown across a variety of scientific disciplines including financial modeling \citep{joy1996quasi,lai1998applications,l2004quasi,l2009quasi,xu2015high,giles.mlqmc_path_simulation}, solving PDEs with random coefficients \citep{graham2011quasi,kuo2012quasi,kuo2015multi,graham2015quasi,MagicPointShop.software,robbe.multi_index_qmc}, and light transport for graphics rendering with ray tracing \citep{jensen2003monte,raab2006unbiased,waechter2011quasi} among others. Randomizing LD sequences provides additional advantages, such as enabling practical QMC error estimation, avoiding boundary observations, and negating the probability of pairing to adverse functions \citep{owen1995randomly,l2016randomized,l2023confidence,owen.variance_alternative_scrambles_digital_net,MATOUSEK1998527,tezuka2002randomization}.

LD sequences, especially randomized versions, have also been extensively used as efficient experimental designs. In reproducing kernel Hilbert spaces (RKHSs), pairing certain LD points with certain kernels yields structured Gram matrices which are diagonalizable by fast transforms such as the fast Fourier transform (FFT) \citep{zeng.spline_lattice_digital_net,zeng.spline_lattice_error_analysis}. These structures have been exploited to accelerate kernel interpolation methods for solving PDEs with random coefficients \citep{kaarnioja.kernel_interpolants_lattice_rkhs,kaarnioja.kernel_interpolants_lattice_rkhs_serendipitous,sorokin.gp4darcy}, Bayesian cubature methods \citep{rathinavel.bayesian_QMC_lattice,rathinavel.bayesian_QMC_sobol,rathinavel.bayesian_QMC_thesis}, and discrepancy measure computations \citep{hickernell.generalized_discrepancy_quadrature_error_bound,hickernell1998lattice}. LD points have also been used to efficiently train neural networks in scientific machine learning \citep{longo2021higher,chen2021quasi,keller2025regularity}; this avenue is not described further in this article.

While randomized LD sequences have been extensively studied for QMC and fast kernel methods, implementations of such methods are scattered or even missing in some popular programming languages. Our prior works \citep{choi.QMC_software,choi.challenges_great_qmc_software,hickernell2025quasi,sorokin.MC_vector_functions_integrals} described the current QMC software landscape and introduced \texttt{QMCPy}\footnote{\url{https://qmcsoftware.github.io/QMCSoftware/}}: a community driven implementation of QMC routines into a unified and accessible Python framework. 

\subsection{\texttt{QMCPy} Features}

Below we summarize the supported features in \texttt{QMCPy}:
\begin{description}
    \item[Randomized LD Sequences] We support the point sets and randomization routines described below. These features are also supported in the comprehensive C++ \texttt{LatNet Builder}\footnote{\url{https://github.com/umontreal-simul/latnetbuilder}} software \citep{LatNetBuilder.software}. \texttt{LatNet Builder} and the multi-language \texttt{Magic Point Shop (MPS)}\footnote{\url{https://people.cs.kuleuven.be/~dirk.nuyens/qmc-generators/}} \citep{MagicPointShop.software} both provide search routines for finding good generating vectors and matrices which respectively define lattices and digital nets. \texttt{QMCPy} integrates with the new \texttt{LDData}\footnote{\url{https://github.com/QMCSoftware/LDData}} repository which contains a variety of these pregenerating vectors and matrices in standardized formats. \texttt{LDData} additionally includes the popular choices from the websites of Frances Kuo on lattices\footnote{\url{https://web.maths.unsw.edu.au/~fkuo/lattice/index.html}} \citep{cools2006constructing,nuyens2006fast} and Sobol' points\footnote{\url{https://web.maths.unsw.edu.au/~fkuo/sobol/index.html}} \citep{joe2003remark,joe2008constructing}.
    \begin{description}
        \item[Lattice Points] Lattices may be generated in linear, radical inverse, or Gray code order, and may be randomized using shifts modulo one. Randomly shifted lattices are also available in \texttt{MPS} and MATLAB's \texttt{GAIL}\footnote{\url{http://gailgithub.github.io/GAIL_Dev/}} (Guaranteed Automatic Integration Library) \citep{GAIL.software,hickernell2018monte}.
        \item[Digital Nets in base $2$] Digital nets, including higher-order digital nets, may be generated in either radical inverse or Gray code order, and may be randomized with linear matrix scrambling (LMS) \citep{owen.variance_alternative_scrambles_digital_net}, digital shifts, nested uniform scrambling (NUS) \citep{owen1995randomly}, and/or permutation scrambling. Early implementations of unrandomized digital sequences, including the Faure, Sobol', and Niederreiter--Xing constructions, can be found in \citep{fox1986algorithm,bratley1992implementation,bratley2003implementing,pirsic2002software}. Considerations for implementing scrambles were discussed in \citep{hong2003algorithm}. Support for combining LMS with digital shifts is provided in MATLAB \citep{MATLAB}, \texttt{MPS}, and both the \texttt{PyTorch} \citep{paszke2019pytorch} and \texttt{SciPy} \citep{virtanen2020scipy} Python packages.
        \item[Halton Points] As with digital nets, Halton point sets may be randomized with LMS, digital shifts, NUS, and/or permutation scrambles. The implementation of Halton point sets and randomizations have been treated in \citep{owen_halton,wang2000randomized}. The \texttt{QRNG}\footnote{https://cran.r-project.org/web/packages/qrng/qrng.pdf} (Quasi-Random Number Generators) R package \citep{qrng.software} implements generalized Halton point sets \citep{faure2009generalized} which use optimized digital permutation scrambles; these are also supported in \texttt{QMCPy}.
    \end{description}
    \item[Variable Transformations] These define the distribution of stochasticity in the underlying problem and  automatically rewrite user-defined functions into QMC-compatible forms. These transforms are mainly wrappers around distributions provided in the \texttt{SciPy} Python package.  
    \item[Monte Carlo Error Estimators] Our adaptive error estimation algorithms automatically select the number of LD points required for a QMC approximation to be within user-specified error tolerances. Available stopping criterion are described below. The single level algorithms are implemented in MATLAB's \texttt{GAIL}.
    \begin{description}
        \item[Monte Carlo with Independent Points] Confidence intervals are derived from either a central limit theorem heuristic or the the guaranteed version for functions with bounded kurtosis \citep{hickernell.MC_guaranteed_CI}.
        \item[QMC with Multiple Randomizations] The confidence interval is derived from a Student's $t$ confidence intervals \cite{l2023confidence}. 
        \item[QMC via Decay Tracking] Using only a single randomized LD sequence, guaranteed error bounds are available for cones of functions whose Fourier coefficients or Walsh coefficients decay predictably \cite{hickernell.adaptive_dn_cubature,adaptive_qmc,cubqmclattice,ding2018adaptive}. 
        \item[QMC via Bayesian Cubature] Using only a single randomized LD sequence, one may quickly performing Bayesian cubature using fast kernel computations \citep{rathinavel.bayesian_QMC_lattice,rathinavel.bayesian_QMC_sobol,rathinavel.bayesian_QMC_thesis}.
        \item[Multilevel Monte Carlo with Independent Points] We have implemented the standard Multilevel Monte Carlo \citep{giles.MLMC_path_simulation} and continuation multilevel Monte Carlo algorithms \citep{robbe2016dimension,robbe2019recycling}.  
        \item[Multilevel Quasi-Monte Carlo] We have implemented the standard Multilevel QMC \citep{giles.mlqmc_path_simulation} and continuation multilevel QMC algorithms \citep{robbe.multi_index_qmc}. 
    \end{description}
    \item[Fast Kernel Computations] Matching a lattice in radical inverse order to a shift-invariant (SI) RKHS kernel yields a Gram matrix diagonalizable by the FFT in bit-reversed order (FFTBR) and its inverse (IFFTBR). Similarly, matching a digital net in radical inverse order to a digitally-shift-invariant (DSI) kernel yields a Gram matrix diagonalizable by the fast Walsh--Hadamard transform (FWHT) \citep{fino.fwht}. The  currently supported kernels and fast transforms are described below.
    \begin{description}
        \item[RKHS Kernels] We provide SI kernels and DSI kernels, including those of higher-order smoothness. We also maintain an interface to a number of standard kernels, such as squared exponential and Mat\'ern kernels, whose implementations may also be found in \texttt{GPyTorch} \citep{gardner.gpytorch_GPU_conjugate_gradient} and the \texttt{scikit-learn} \citep{scikit-learn} Python packages among others. 
        \item[Fast Transforms] We provide interfaces to FFTBR, IFFTBR, and FWHT fast transform algorithms which all have $\mathcal{O}(n \log n)$ complexity in the number of points $n$. The FFTBR and IFFT implementations use the FFT routines in the \texttt{SciPy} and \texttt{PyTorch} Python packages. An implementation of the FWHT is also available in the \texttt{SymPy} \citep{10.7717/peerj-cs.103} Python package.
    \end{description}
\end{description}

\subsection{Novel Contributions and Scope}

With respect to prior work, we provide:
\begin{enumerate}
    \item A more complete Python interface to randomized LD sequences, variable transformations, Monte Carlo and Quasi-Monte Carlo error estimators, and fast kernel computations.
    \item The first Python interface to support digital interlacing for the creating of higher order digital nets and higher order scrambling with either LMS or NUS. 
    \item A new digitally-shift-invariant kernels of higher-order smoothness, including an order $4$ smoothness DSI kernel whose form does not appear in the literature.
    \item Novel routines to efficiently update Gram matrix products and inverses after doubling the number of lattice or digital net points.
\end{enumerate}

With respect to our prior works \citep{choi.QMC_software,choi.challenges_great_qmc_software,hickernell2025quasi,sorokin.MC_vector_functions_integrals}, we provide: 
\begin{enumerate}
    \item 
\end{enumerate}
\subsection{Outline} 
