...
\keywords{Low-discrepancy sequences, low-discrepancy pointset randomizations, higher-order digital net scrambling, randomized Quasi-Monte Carlo, fast kernel methods, shift-invariant kernels, digitally-shift-invariant kernels}
...
%\FJHNote{[Since ACM-TOMS has some of quasi-MC algorithms, you should reference them and explain what you are adding.] } 
...
\begin{description}
    ...
    %\FJHNote{[Does it make sense to include other randomizations of Halton, such as random start and I forget what Christiane does?] }\AGSNote{Random start Halton chooses a random starting index? Christiane does deterministic permutation scrambles. Both are supported.} We are among the first to provide a Python interface for such randomizations.  
...
%\FJHNote{[Is this what can be used in QMCPy?  The code does not work for me.]} \AGSNote{The \texttt{replications} parameter is added in the Pull Request \url{https://github.com/QMCSoftware/QMCSoftware/pull/362}.} 
...
%Our implementation is separated into base $b=2$ digital nets and generalized digital nets. This is done because base $b=2$ digital nets can store columns of generating matrices as integers and then quickly perform digital addition using exclusive or (XOR) instructions. \texttt{QMCPy} currently only supports Halton pointsets, although this is a topic for future work. 
